...
class Carrousel {
  ...
  injecterHTML() {
    ...
    this.diapoConteneur = this.elementHTML.querySelector(
      ".carrousel__conteneur_diapo"
    );
   ...
   */
  onClickPrecedentDiapo(evenement) {
    let declencheur = evenement.currentTarget;
    declencheur.classList.add("animation");
  ...
  demarrer() {
    ...
    this.interval = setInterval(this.prochaineDiapo.bind(this), 2000);
  }
  ...
  afficherDiapo(index) {
    ...
    this.diapoActuelle = new CarrouselImage(
      this,
      src,
      alt,
      this.diapoConteneur
  ...
  formatterAlbumSrc(album) {
    return `assets/img/albums/${album.album
      .toLowerCase()
      .replace(" ", "_")}.webp`;
  }
